...
```bash
bun add git+ssh://git@github.com/rekkrjs/orm.git#v1.3.0
```
...
not use `bun add @rekkr/orm` yet: the registry package is not the `v1.3.0`
...
bun add git+ssh://git@github.com/rekkrjs/orm.git#<tag>          # e.g. v1.3.0
...
git ls-remote git@github.com:rekkrjs/orm.git refs/tags/v1.3.0
```
